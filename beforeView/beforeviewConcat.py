...
import beforeView.beforeviewCenter as viewCenter
...
def calculate(ind):
    ...
    for sample_index in range(sample_amount):
        sample_predict, dogfall = f(ind, sample_index)  #返回样本预测标签，以及是否出现平局（2 未出现；1 出现）
...
def concat_acc(ind_list):
    ...
    for c in concat_list:
        ...
        print("\nview concat", c)
        t_acc, t_cover, t_c1, t_c2, t_e1, t_e2, evaluate_l2 = calculate(ind)
